...
This module defines the API endpoints for managing blog posts.
...
def register(
    ...
    Args:
        ...
        payload (UserRegistration): The input data containing username and
                                                                password.

    Returns:
        UserResponse: The created user data, including its ID and username.
...
def login(
    ...
    Args:
        request: The HTTP request object.
        payload (UserLogin): The input data containing username and password.

    Returns:
        Token: The JWT token used for user authentication.
        or
        dict: An error message if the credentials are invalid.
...
def create_post(
    ...
    Args:
        ...
        payload (PostIn): The data required to create the post, including
                          title, content, and automatic reply settings.

    Returns:
        JsonResponse: A response containing the created post's ID, title,
                      content, creation timestamp, and block status.
...
def list_posts(request: Any) -> List[PostOut]:
    ...
    Returns:
        List[PostOut]: A list of blog posts, each including ID, title,
                        and content
...
def create_comment(request: Any,
    ...
    Args:
        ...
        payload (CommentIn): The input data containing the content of the
                                                                comment.

    Returns:
        CommentOut: The created comment data, including its ID, post ID,
                     content, author ID, creation date, and blocked status.
...
def list_comments(
    ...
    Returns:
        List[CommentOut]: A list of comments for the specified post, each
                        including ID, content, author ID, creation date,
                        and blocked status.
...
def comments_daily_breakdown(request: Any, 
    ...
    Args:
        ...
        request: The HTTP request object containing user information.
        date_from (str): The start date for the comment filter, formatted as
                         YYYY-MM-DD.
        date_to (str): The end date for the comment filter, formatted as
                       YYYY-MM-DD.

    Returns:

        dict: A dictionary containing:
            1. total_comments (int): The total number of comments
                                                in the date range.
            2. blocked_comments (int): The number of comments that are blocked.
