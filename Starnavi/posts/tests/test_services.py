"""
Tests for JWT Authentication and Content Moderation Utilities

This module contains unit tests for the services provided in the
posts application, specifically focusing on JWT token creation,
content moderation, and automatic reply functionalities.

It uses pytest and Django's testing utilities to validate the
correctness and functionality of the service methods.

Tests:

1. test_create_jwt_token: Verifies that a JWT token is generated
  successfully for a given user.

2. test_moderate_content_with_profanity: Checks the functionality
  of the content moderation utility to detect profane language.

3. test_send_auto_reply: Ensures that an automatic reply is sent
  correctly to a comment based on the associated post settings.
