"""
Tests for Pydantic Schemas in Blog Application

This module contains unit tests for validating the Pydantic schemas used
in the blog application. These tests ensure that the schemas behave as
expected, both for valid input/output data and for scenarios where
invalid data is provided.

Schemas Tested:

1. PostIn: Schema for input data when creating a new blog post.
2. PostOut: Schema for output data representing a blog post.
3. CommentIn: Schema for input data when creating a new comment.
4. CommentOut: Schema for output data representing a comment.
5. UserRegistration: Schema for input data when registering a new user.
6. UserResponse: Schema for output data after user registration.
7. UserLogin: Schema for input data when a user logs in.
8. Token: Schema for output data representing a JWT token
                            used for user authentication.

Tests Included:

1. test_post_in_schema: Verifies the PostIn schema with valid data.
2. test_post_out_schema: Verifies the PostOut schema with valid data.
3. test_comment_in_schema: Verifies the CommentIn schema with valid data.
4. test_comment_out_schema: Verifies the CommentOut schema with valid data.

5. test_user_registration_schema: Verifies the UserRegistration schema
                                                        with valid data.

6. test_user_response_schema: Verifies the UserResponse schema with valid data.
7. test_user_login_schema: Verifies the UserLogin schema with valid data.
8. test_token_schema: Verifies the Token schema with valid data.

9. test_post_in_schema_invalid: Ensures the PostIn schema raises a validation
                                                    error with invalid data.

10. test_user_registration_schema_invalid: Ensures the UserRegistration schema
raises a validation error with invalid data.
